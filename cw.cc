...
ofstream outfile ("scratch/RLinWiFi-master-original-Desc-Test1.3/CW_data.csv", fstream::out);
...
uint64_t g_rx2PktNum = 0;
uint64_t g_tx2PktNum = 0;
...
bool MyExecuteActions(Ptr<OpenGymDataContainer> action)
{
    ...
    if (type == "discrete")
    {
        ...
        //std::cout <<" CW discrete: " << CW << endl;
    }
    else if (type == "continuous")
    {
        CW = pow(2, actionVector.at(0) + 4);
        //std::cout <<" CW continuous: " << CW << endl;
    }
...
Ptr<OpenGymDataContainer> MyGetObservation()
{
    ...
    for (uint32_t i = 0; i < history_2.size(); i++)
    {
        if (history_2[i] >= -100 && history_2[i] <= 100)
            box2->AddValue(history_2[i]);
        else
            box2->AddValue(0);
    }
    for (uint32_t i = history_2.size(); i < history_length; i++)
    {
        box2->AddValue(0);
    }
    ...
    std::vector<uint32_t> shapeT = {nodeNum,};
    Ptr<OpenGymBoxContainer<uint32_t> > boxT = CreateObject<OpenGymBoxContainer<uint32_t> >(shapeT);
    ...
    //Observation box posicao 0 - Agent 1 
    Ptr<OpenGymBoxContainer<uint32_t> > mbox = DynamicCast<OpenGymBoxContainer<uint32_t> >(data->Get(0));
    
    //Observation box posicao 1 - Agent 2
    //Ptr<OpenGymBoxContainer<uint32_t> > mbox2 = DynamicCast<OpenGymBoxContainer<uint32_t> >(data->Get(1));
    ...
    //std::cout <<" MyGetObservation: " << data << endl;
    //std::cout <<"---" << mbox << endl;
    //std::cout <<"---" << mdiscrete << endl;
...
void recordHistory_2()
{
    // Keep track of the observations
    // We will define them as the error rate of the last `envStepTime` seconds
    static uint32_t last_rx2 = 0;            // Previously received packets
    static uint32_t last_tx2 = 0;            // Previously transmitted packets
    static uint32_t calls2 = 0;              // Number of calls to this function
    calls2++;
    current_time += envStepTime;

    float received2 = g_rx2PktNum - last_rx2;  // Received packets since the last observation
    float sent2 = g_tx2PktNum - last_tx2;      // Sent (...)
    float errs2 = sent2 - received2;           // Errors (...)
    float ratio2;

    ratio2 = errs2 / sent2;
    //std::cout <<" Node Pcol: " << ratio << endl;
    history_2.push_front(ratio2);

    // Remove the oldest observation if we have filled the history
    if (history_2.size() > history_length)
    {
        history_2.pop_back();
    }

    // Replace the last observation with the current one
    last_rx2 = g_rx2PktNum;
    last_tx2 = g_tx2PktNum;

    if (calls2 < history_length && non_zero_start)
    {   
        // Schedule the next observation if we are not at the end of the simulation
        Simulator::Schedule(Seconds(envStepTime), &recordHistory_2);
    }
    else if (calls2 == history_length && non_zero_start)
    {
        g_rx2PktNum = 0;
        g_tx2PktNum = 0;
        last_rx2 = 0;
...
void packetReceived_2(Ptr<const Packet> packet)
{
    NS_LOG_DEBUG("Client received a packet of " << packet->GetSize() << " bytes");
    g_rx2PktNum++;
}

void packetSent_2(Ptr<const Packet> packet)
{
    g_tx2PktNum++;
}
...
{
    std::cout << Simulator::Now().GetSeconds() << ": Start sending "<<*p<<"\n";
    g_txPktNum++;
}*/
 ...
 */
    ...
    /*
    Agent 2 adicionado por sheila
    */
    wifiScenario->installScenario(simulationTime + end_delay + envStepTime, envStepTime, MakeCallback(&packetReceived_2));

     Config::ConnectWithoutContext("/NodeList/0/ApplicationList/*/$ns3::OnOffApplication/Tx", MakeCallback(&packetSent1));
     
     //Config::ConnectWithoutContext("/NodeList/1/ApplicationList/*/$ns3::OnOffApplication/Tx", MakeCallback(&packetSent_2));
     
     // quando utilizamos /NodeList/0/DeviceList/*/ na instrução da linha seguir ela apresenta valores nagativos ex: -0.5, -5.2, -3.833
    //Config::ConnectWithoutContext("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Phy/PhyTxBegin", MakeCallback(&packetSent)); 
    
    //Config::Connect("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Phy/PhyTxBegin", MakeCallback(&packetSent)); // Funcionou
    
    //Config::Connect("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Phy/PhyTxBegin", MakeCallback(&packetSent)); // gera pcol com números negativos ex: -5
    
    //Config::Connect ("/NodeList/0/DeviceList/*/$ns3::WifiNetDevice/Phy/PhyTxBegin",MakeCallback(&PhyTxBegin)); // gera pcol com números negativos ex: -5
