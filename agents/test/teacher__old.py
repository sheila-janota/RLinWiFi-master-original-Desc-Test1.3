...
class Logger:
    ...
    def log_round(self, states, reward, cumulative_reward, info, loss, observations, step):
        ...
        round_mb = info[0]
        #print("Teacher round_mb", round_mb)  
        CW = info[1]
        #print("Teacher cw value", CW) 
        # stations = np.mean([float(i.split("|")[2]) for i in info])
        self.stations = info[2]
        #print("Teacher print info", info)
        #print("Teacher stations Info", self.stations)
        fairness = info[3]
        #print("Teacher print Fairnesss", fairness)
        #print("Teacher  print Reward", reward)
        #print("Teacher  print cumulative_reward", cumulative_reward)
         # acumular o speed/throughput de todas as estações e fazer np.mean do speed
    ...
    def log_episode(self, cumulative_reward, speed, step):
        ...
        print("Teacher last_speed", self.last_speed)
        
        for i in (self.stations):
             self.last_speed1 += speed
             print("Teacher last_speed1", self.last_speed1)
        self.speed_window = deque(maxlen=100)
        self.current_speed = 0
...
class EnvWrapper:
    ...
    def _craft_commands(self, params):
        ...
        command = f'{waf_pwd} --run "RLinWiFi-master-original-Desc-Test1.3'
